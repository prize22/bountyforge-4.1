...
## Step 1 G현 Enumerate every boundary
...
## Step 2 G현 For every external call: four corner cases
...
2. **Non-standard token.** Void return (USDT-style) breaks `require(token.transferFrom())`. Fee-on-transfer makes received amount G吟 requested amount. Rebasing makes cached balance stale. Blacklist/pausable makes standard transfers revert unexpectedly. Some tokens revert on zero-value approval.
3. **Empty / zero / max input.** Zero amount G현 does the code skip, revert, or proceed wrongly? Empty bytes G현 does abi.decode revert? Max uint G현 does the math overflow before the check?
...
5. **Sentinel-placeholder used in IERC20 op.** Native-token placeholder addresses (`_ETH_ADDRESS_`, `address(0xeee...)`) flow into raw `IERC20(token).approve(...)` and revert because the placeholder has no contract code. For every sentinel-branch, walk forward G현 any downstream `IERC20` op on the same `token` is broken.
6. **False-returning ERC20.** Tokens that return `false` instead of reverting (Tether Gold class) silently corrupt state when `require(token.transfer(...))` is omitted. Distinct from USDT-style void return G현 both must be checked.
...
For every call site that fails any of the questions in a way the calling code doesn't account for G현 finding.

## Step 3 G현 For every payable function: three branch cases

For each `payable` function:

1. `msg.value > 0` G현 is the value spent, refunded, or forwarded? Where does it end up?
2. `msg.value == 0` G현 does the operation still proceed when no native was sent? Does it skip a fee that should have been paid? Does it pull tokens it shouldn't?
3. `msg.value != amount` (when both exist as inputs) G현 is the relationship between `msg.value` and an `amount` parameter enforced? `msg.value > amount` (excess stuck in contract). `msg.value < amount` (under-payment proceeds while accounting believes amount was paid).
...
## Step 4 G현 For every sentinel-address branch: walk both sides
...
3. The branch is your enumeration, not a comparison G현 for each branch, what does this specific path do under inputs the developer didn't anticipate?

## Step 5 G현 For every bytes input / abi.decode: corruption cases

For every `bytes` input or `abi.decode`:
1. Empty input G현 does the code panic? Bypass a loop? Return defaults that look like valid empty state?
2. Length-prefixed array where the length is attacker-supplied G현 attacker writes a length larger than the buffer; OOB reads return zero-padded or trailing bytes.
3. `bytes20(longerBytes)` cast G현 silent truncation. Source can be longer than 20 (BTC bech32, Solana 32-byte, attacker-chosen length).
4. `abi.encodePacked` followed by `abi.decode` G현 packed encoding is ambiguous; decode returns wrong field boundaries.
5. Field-order mismatches across encode and decode sites in different files G현 silent reinterpretation of attacker bytes.
