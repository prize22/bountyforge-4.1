...
>
> - **In Step 2e (Protocol Classification)** GÇö use the detection-signals table to label the protocol by type.
> - **In Step 3a (Writing Section 2 of x-ray.md)** GÇö use adversary rankings, attack patterns, and critical invariants listed here to know *what to look for* and *who threatens the protocol*, then TRANSLATE that knowledge into the output format.
>
> **DO NOT copy exploit-chain prose verbatim into Key Attack Surfaces.** Phrases like *"Oracle manipulation GåÆ inflated collateral GåÆ drain the pool"* are intentional here GÇö they teach the threat GÇö but the `templates.md` **DO-NOT-EXPLOIT RULE** forbids them in the report. Convert "GåÆ attacker drains X" into "worth tracingGÇª" / "worth checkingGÇª" / "worth confirmingGÇª" when writing the bullet. Name the surface and the concern; let the auditor finish the sentence.
...
Detect protocol type from function signatures, state variables, and architectural patterns found during source file reading in Step 2. A protocol may match **multiple types** (hybrid). Rank by signal density GÇö the type with the most matches is primary.
...
1. Rank by signal count GÇö more matches = higher weight in threat model
...
Example: A protocol with `swap()`, `addLiquidity()`, `borrow()`, `liquidate()` GåÆ Primary: DEX/AMM, Secondary: Lending/Borrowing.
...
1. **Flash loan attacker** GÇö Borrows unlimited capital in a single transaction to manipulate oracle prices, inflate collateral values, and drain borrow capacity. Flash loans reduce the cost of oracle manipulation to near-zero.
2. **Oracle manipulator** GÇö Manipulates price feeds (spot or TWAP) to make collateral appear more valuable or debt appear less valuable. The oracle is the single source of truth for solvency GÇö if it lies, everything downstream breaks.
3. **Liquidation MEV searcher** GÇö Extracts value from liquidation events through front-running, back-running, or sandwich attacks. If MEV extraction makes liquidation unprofitable for honest liquidators, bad debt accumulates.
4. **Malicious first depositor** GÇö In protocols with share-based accounting (supply tokens, debt tokens), the first depositor can manipulate the share price to steal from subsequent depositors. Classic vault inflation attack applied to lending pools.
5. **Compromised admin** GÇö Can change collateral factors, oracle addresses, interest rate models, or pause liquidations. Any of these can instantly make the protocol insolvent or prevent it from recovering.

**Dominant attack patterns:**
- Oracle manipulation GåÆ inflated collateral value GåÆ max borrow GåÆ drain lending pool
- Flash loan borrow GåÆ manipulate spot price GåÆ liquidate victim at wrong price GåÆ profit from liquidation bonus
...
- Recursive borrowing: deposit collateral GåÆ borrow GåÆ deposit borrowed asset as collateral GåÆ borrow again GåÆ amplified exposure that collapses under price movement

**Critical invariants:**
- `totalBorrows <= totalCollateral * LTV` GÇö always, for every market and every account
...
1. The complete price calculation path: oracle read GåÆ price normalization GåÆ collateral value GåÆ health factor. Every step is a manipulation point.
...
1. **MEV searcher / sandwich attacker** GÇö The dominant threat to DEX users. Monitors mempool for pending swaps, inserts transactions before and after to extract value. Every swap without adequate slippage protection is a guaranteed extraction opportunity.
2. **Flash loan price manipulator** GÇö Uses flash-loaned capital to move pool prices within a single transaction.
3. **Malicious first LP / empty pool attacker** GÇö Manipulates pool initialization or empty-state transitions. In concentrated liquidity: can set initial tick to a manipulated price. In constant-product: can inflate LP share price through donation before the first real deposit.
4. **Liquidity manipulation attacker** GÇö Adds and removes liquidity strategically to extract value from other LPs.
5. **Compromised admin** GÇö Can change fee structures, pause trading, modify routing, or whitelist malicious pools.

**Dominant attack patterns:**
- Sandwich attacks: front-run swap to move price GåÆ victim swaps at worse price GåÆ back-run to capture difference
...
**Primary adversaries** (ranked):
1. **Share inflation attacker (first depositor)** GÇö The canonical vault attack. Deposit 1 wei, donate a large amount directly to the vault (inflating `totalAssets` without minting shares), then when the next user deposits, they receive 0 shares due to rounding and the attacker redeems for the donated + deposited amount.
2. **Malicious/compromised strategy** GÇö Strategies hold the actual funds. A malicious strategy can report fake losses, retain approvals after migration, or transfer funds out.
3. **Reentrancy through external protocol callbacks** GÇö Vault deposits into Aave/Compound/Yearn, which may trigger callbacks during deposit/withdraw. If vault state is inconsistent during the callback window, reentrancy can manipulate share prices.
4. **Donation/direct-transfer attacker** GÇö Sends tokens directly to the vault contract (not through deposit()) to manipulate `totalAssets()` and therefore share price. If `totalAssets` reads `balanceOf(address(this))`, any donation changes the share price.
5. **Compromised admin** GÇö Can add malicious strategies, change allocation weights, set harvester address, or migrate funds to attacker-controlled strategy.

**Dominant attack patterns:**
- ERC4626 share inflation: deposit(1) GåÆ donate(large amount) GåÆ next depositor gets 0 shares GåÆ redeem(all)
- Strategy reports fake gain GåÆ inflated share price GåÆ attacker deposits at inflated price GåÆ strategy reports real value GåÆ attacker loses nothing, previous depositors diluted
- Strategy retains token approval after migration to new strategy GÇö old strategy can still pull funds
...
- `convertToShares(convertToAssets(shares)) <= shares` GÇö round-trip must not create value
- `convertToAssets(convertToShares(assets)) <= assets` GÇö same in reverse
...
**Primary adversaries** (ranked):
1. **Oracle manipulator** GÇö If collateral price is manipulated upward, attacker can mint stablecoins against less real collateral. If manipulated downward, legitimate positions get liquidated at unfair prices. In algorithmic stablecoins, oracle manipulation can trigger or amplify depegs.
2. **Economic/governance attacker** GÇö Acquires governance power to change collateral parameters (lower ratios, add risky collateral, change stability fees) to extract value or destabilize the peg. Can also manipulate stability mechanisms.
3. **Bank run attacker** GÇö Triggers mass redemption by creating panic or exploiting information asymmetry. If the stablecoin's redemption mechanism has capacity limits, a strategic redemption can drain the best collateral, leaving remaining holders with worse backing.
4. **Flash loan minter** GÇö Flash loans capital to mint stablecoins, manipulates collateral price, and profits from the discrepancy. Especially dangerous if minting has no cooldown or rate limit.
5. **Compromised admin** GÇö Can change collateral types, oracle addresses, debt ceilings, stability fees, or pause redemptions. Any of these can break the peg or trap user funds.

**Dominant attack patterns:**
- Collateral price manipulation GåÆ mint at inflated collateral value GåÆ sell stablecoins GåÆ collateral price returns to normal GåÆ protocol is undercollateralized
- Algorithmic death spiral: sell pressure GåÆ depeg GåÆ collateral value drops GåÆ more liquidations GåÆ more sell pressure GåÆ repeat (LUNA/UST)
...
**What to look for first:**
1. Minting path: what collateral is accepted GåÆ how is it valued (oracle) GåÆ what's the ratio GåÆ can the ratio be changed?
...
**Primary adversaries** (ranked):
1. **Oracle manipulator** GÇö In derivatives, oracle errors are amplified by leverage. A 1% oracle manipulation on a 50x leveraged position creates a 50% PnL swing.
2. **Liquidation MEV searcher** GÇö Extracts value from liquidation events. In perps, positions can be large and leverage amplifies the liquidation bonus. May also manipulate price to trigger liquidations, then capture the liquidated collateral.
3. **Funding rate manipulator** GÇö Skews open interest to force favorable funding rate payments. With enough capital, can make the funding rate so extreme that opposing positions are forced to close, then reverse to capture the funding.
4. **Position size attacker** GÇö Opens positions larger than the protocol can pay out, or opens positions across multiple accounts to circumvent limits. If the protocol's liquidity pool cannot cover max payout, insolvency results.
5. **Compromised admin** GÇö Can change max leverage, funding rate parameters, liquidation thresholds, or oracle addresses. Can also pause liquidations (creating bad debt) or enable instant position changes that bypass risk checks.

**Dominant attack patterns:**
- Oracle manipulation GåÆ cascade liquidation GåÆ profit from liquidated positions
...
- Delayed/stale oracle GåÆ risk-free directional bet (see current price off-chain, trade at stale on-chain price)
...
**Primary adversaries** (ranked):
1. **Exchange rate manipulator** GÇö The derivative token's value depends on an exchange rate (stETH/ETH, rETH/ETH). If this rate can be manipulated (through rewards reporting, slashing events, or direct donation), attackers can buy/sell the derivative at wrong prices against protocols that use it as collateral.
2. **Validator set attacker** GÇö Compromises or controls validators that the protocol delegates to. Can trigger slashing events, withhold rewards, or censor transactions. The trust model around validator selection is critical.
3. **Withdrawal queue attacker** GÇö Exploits timing or ordering in the unstaking queue. May front-run large unstake requests to exit first, or manipulate queue mechanics to delay others' withdrawals.
4. **Oracle/rate arbitrageur** GÇö Exploits lag between the on-chain exchange rate and real underlying value. When a slashing event occurs, the on-chain rate may not update immediately GÇö attacker sells derivative at stale (higher) rate before the slash is reflected.
5. **Compromised admin** GÇö Can change validator set, fee parameters, oracle addresses, or withdrawal mechanisms. Can also pause withdrawals, trapping user funds.
...
**Primary adversaries** (ranked):
1. **Validator/relayer set attacker** GÇö Compromises the threshold of validators/relayers needed to approve cross-chain messages. This is the #1 bridge exploit vector by total value lost.
2. **Message replay attacker** GÇö Replays a valid cross-chain message on a different chain or replays the same message multiple times to mint/unlock tokens repeatedly.
3. **Race condition exploiter** GÇö Exploits timing gaps between source and destination chain finality. Initiates action on source chain, front-runs the relay on destination chain, or exploits reorgs to reverse source chain action after destination chain has already processed it.
4. **Fake message crafter** GÇö Crafts a cross-chain message that passes validation but contains malicious data. Exploits weaknesses in message encoding, proof verification, or chain ID validation.
5. **Compromised admin** GÇö Can change validator set, pause bridge (trapping funds), or upgrade contracts to drain locked funds. Bridge admin keys are the highest-value targets in DeFi.

**Dominant attack patterns:**
- Validator key compromise GåÆ forge cross-chain messages GåÆ mint unbacked tokens on destination
...
- Reorg exploitation: deposit confirmed on source chain GåÆ relayed to destination GåÆ source chain reorgs GåÆ deposit reversed but destination tokens already minted
...
**Primary adversaries** (ranked):
1. **Flash loan governance attacker** GÇö Borrows governance tokens via flash loan, votes on a proposal, and returns tokens in the same transaction. Only possible if voting power is measured at current block rather than a snapshot.
2. **Governance capture attacker** GÇö Gradually accumulates voting power (buying tokens, borrowing from lending protocols, receiving delegations) to pass malicious proposals. Patient, multi-block attack with potentially massive payoff.
3. **Proposal spam / griefing attacker** GÇö Submits many proposals to exhaust voter attention, or submits proposals that appear benign but have hidden malicious effects (e.g., "update parameter to X" where X causes insolvency).
4. **Timelock exploitation attacker** GÇö Monitors queued proposals and positions to exploit parameter changes the instant they execute.
5. **Compromised admin/guardian** GÇö Can cancel proposals, pause governance, or execute emergency actions that bypass normal governance flow.

**Dominant attack patterns:**
- Flash loan GåÆ vote GåÆ return: instant governance control if no snapshot
...
| Phase | Include When |
|-------|-------------|
| **Deployment & Initialization** | Always include GÇö every protocol has this phase |
| **Steady State** | Always include GÇö this is the baseline |
...
Normal operation. This is where the existing adversary types (flash loan, MEV, external user, compromised admin) operate. The standard threat model covers this phase GÇö no additional temporal-specific content needed. The protocol-type threat profiles provide the detailed guidance for this phase.
...
What to look for: Chainlink `latestRoundData()` calls GÇö what staleness threshold is used? Is it appropriate for the asset's volatility? Is the heartbeat period documented/configured or hardcoded? Is there a deviation threshold check? What happens if `updatedAt` is 0 or in the future?

**Liquidation cascade:**
Position A is liquidated GåÆ liquidation dumps collateral on market GåÆ price drops further GåÆ Position B is liquidated GåÆ cycle repeats. The protocol's own liquidation mechanism amplifies the crash. Can cause systemic insolvency.

What to look for: Liquidation mechanism GÇö does it sell collateral on-market (creating price impact)? Is there a circuit breaker? Is liquidation throttled? Can the protocol handle 30%+ collateral price drops in a single block?
...
What to look for: Liquidation profitability assumptions GÇö are they valid when liquidity is thin? Does the protocol assume swap paths exist with sufficient depth? Is there a minimum liquidity requirement?
...
What to look for: Timelock durations GÇö are they long enough for users to react? Can users exit positions before parameter changes take effect? Are there parameters that could be exploited if their pending value is publicly known?
...
What to look for: Voting power source GÇö `balanceOf(msg.sender)` (vulnerable) vs. snapshot at proposal creation block (immune). Can governance tokens be borrowed from lending protocols?

**Governance capture (slow):**
Attacker accumulates voting power over time GÇö buying tokens, receiving delegations, borrowing from Aave. Once threshold is reached, passes malicious proposals. The timelock is the last defense.

What to look for: Token distribution GÇö is voting power concentrated? What's the quorum? Can a well-funded attacker buy enough tokens to pass proposals? Is there a guardian that can veto?

**Migration window:**
Protocol migrates from V1 to V2. During migration, funds are in transit. Approval chains exist between old and new contracts. Users who don't migrate lose access or face degraded conditions. The V1GåÆV2 bridge is an attack target.

What to look for: Migration functions, V1GåÆV2 transfer mechanisms. Do V1 contracts retain fund access? Is there a deadline? Can migration be front-run?
...
Users who interacted with V1 still have active token approvals to V1 contracts. If V1 has any exploitable path, those user approvals are a liability GÇö attacker can drain user wallets through the deprecated contract.
...
2. **List the specific threats** that apply (not all threats from every phase GÇö only those where the code has the relevant patterns)
...
Keep it concise GÇö 2-4 bullets per applicable phase. Phase 2 (Steady State) is covered by the main threat model, so skip it in the temporal section to avoid duplication.
# Cross-Protocol Composability Threats

DeFi's unique property is composability GÇö protocols interact with other protocols, creating emergent risks that don't exist in isolated analysis. This reference provides a systematic framework for identifying and documenting composability threats.
...
The protocol directly calls external contracts. These are visible in the code GÇö every `interface` import and external call is a direct dependency.
...
**Threat**: Protocol GåÆ Oracle GåÆ underlying source(s). If any source in the chain is manipulable within the protocol's trust assumptions, the oracle is effectively manipulable.
...
**Threat**: The code assumes standard ERC20 behavior. Non-standard tokens break these assumptions silently GÇö no revert, just wrong accounting.
...
**Threat**: Even with reentrancy guards on direct calls, callbacks through external protocols can bypass them. Token transfer GåÆ external protocol callback GåÆ re-enter through a different function.
...
**Threat**: External protocol deprecates an oracle feed, a pool, or an endpoint. The call doesn't revert GÇö it returns stale/wrong data silently. Or it starts reverting, and this protocol's try/catch falls through to an unsafe default.
