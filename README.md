...
**S-Class Modules (v5.1 Additions):** Five autonomous hunting modules that upgrade BountyForge from a static knowledge base to a self-evolving organism:
- **Shadow Logic** — infers business logic by observing API behavior, detects state machine violations and semantic anomalies
- **Patch-Gap Vampire** — analyzes security patches and hunts for unpatched variants in adjacent code paths
- **Second-Order Ghost Detector** — finds stored-input vulnerabilities processed unsafely by different components (2nd-order XSS, SQLi, SSRF, command injection)
- **Weirdness Scorer** — baselines endpoint behavior and finds statistical outliers suggesting hidden functionality, debug endpoints, or unpatched vulnerabilities
- **Semantic Taint Tracker** — static analysis tracking attacker-controlled input through code to dangerous sinks without passing through validation
